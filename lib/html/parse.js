...
function parse(html) {
	var data = [],
		curEl = { children: data, tagName: 'default' },
			...
			if(cur === '>') {
				...
				if(tag[0] === '!') { // comment or doctype
					...
					if(tagName[0] === '/') { // end tag
						var index; // how many to slice off
						if(curTag.html === false) { // no html: slice of one
							index = -1;
						...
						tags.splice(index);
					} else if(curTag.html !== false) {
						// Create the new element
						curEl = createElement(curEl, tag.replace(/\/\s*$/, ''));
						curTag = tagSpecs[tagName.toLowerCase()] || tagSpecs.default;
						
						if(curTag.endTag === false || /\/\s*$/.test(tag)) {
							curEl = curEl.parentNode;
							curTag = tagSpecs[curEl.tagName.toLowerCase()] || tagSpecs.default;
						} else {
							tags.push(tagName);
						}
					} else {
